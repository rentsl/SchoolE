...
class MainTableViewController: UITableViewController, NSFetchedResultsControllerDelegate ,AvailableOrderslisten,DownLoadImgeProtocol{
    ...
    func getNewAvailableOrders(data: AnyObject) {
        ...
        print(dataJson["data"][0])
        print(dataJson["data"].count)
        
        //获取用户头像
        for i in 0...(dataJson["data"].count-1){
            DownLoadImage.downLoadImageWithURLAndIndex(i, imageURL: (MyURLs.urlDownHeader + dataJson["data"][i]["publisher_avatar"].string!))
        }
